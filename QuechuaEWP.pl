...
# I've looked at the paradigm more and found I might be able to do it in fewer rules.
# September 29, 2016. Home, Athens, GA, while watching 'Suits'.
...
for my $tense (@tenses) {
	for my $subject (@persons) {
		for my $object (@persons) {
			...
			# Block 2
			if ($sMe && !defined $sYou && !defined $oYou && $sPl && $tense eq 'fut') {
				$guess .= "sa";
			} elsif ($sMe && !defined $sYou && !defined $oYou && $tense eq 'fut') {
				$guess .= "saq";
			} elsif ($sMe && !defined $sYou && $tense eq 'fut') {
				$guess .= "sqa";
			} elsif ((($sMe && $sYou && $sPl) || ($oMe && $oYou && $oPl)) && $tense eq 'fut') {
				$guess .= "su";
			} elsif ($tense eq "past") {
				$guess .= "rqa";
			}
			...
			# Block 3
			if ($sMe && !defined $sYou && !defined $oYou && !defined $sPl && $tense eq "fut") {
				$guess .= "";
			...
			# Block 4
			if (!defined $sMe && !defined $sYou && !defined $oYou && $tense eq "past") {
				$guess .= "";
			# These two rules can be combined with a -1 rule.
			} elsif (!defined $sMe) {
				$guess .= "n";
			} elsif ($sYou) {
				$guess .= "n";
			...
			# Block 5
			if (!defined $sMe && !defined $sYou && !defined $oYou && $tense eq "fut") {
				$guess .= "qa";
